...
class LenstronomyLens(StrongLensSystem):
    ...
    def __init__(self, name, zlens, zsource, ddt_vs_dd_samples, mu=None, sigma=None, longname=None, loglikelihood_type="kde", kde_type="scipy_gaussian"):
        StrongLensSystem.__init__(self, name=name, zlens=zlens, zsource=zsource, longname=longname)
...
def log_prior(theta, cosmology):
    ...
    if cosmology == "FLCDM":
        ...
        if 0. <= h0 <= 150. and 0.05 <= om <= 0.5:
            h0_prior = norm(loc=70,scale=30)
            om_prior = norm(loc=0.3,scale=0.1)
            # return 0.0
            return h0_prior.logpdf(h0)+om_prior.logpdf(om)
        else:
            return -np.inf
        ...
        if 0. <= h0 <= 150. and 0.05 <= om <= 0.5 and -2.5 <= w <= 0.5:
            h0_prior = norm(loc=70,scale=30)
            om_prior = norm(loc=0.3,scale=0.1)
            w_prior = norm(loc=-1.5,scale=0.5)
            # return 0.0
            return h0_prior.logpdf(h0)+om_prior.logpdf(om)+w_prior.logpdf(w)
        else:
            return -np.inf
        ...
        if 0. <= h0 <= 150. and 0.05 <= om <= 0.5 and -2.5 <= w0 <= 0.5 and  -2 <= wa <= 2:
            h0_prior = norm(loc=70,scale=30)
            om_prior = norm(loc=0.3,scale=0.1)
            w0_prior = norm(loc=-1.5,scale=1.)
            wa_prior = norm(loc=0,scale=1.)
            # return 0.0
            return h0_prior.logpdf(h0)+om_prior.logpdf(om)+w0_prior.logpdf(w0)+wa_prior.logpdf(wa)
        else:
            return -np.inf
